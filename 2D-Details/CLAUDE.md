...
1. **Read this file end-to-end.** Especially the variable conventions and the file map.
2. **Identify which `js/NN-*.js` file the change belongs in.** If it doesn't fit, that's a signal the file boundaries need a tweak — flag it before adding a new file.
3. **Make the change in `dev/js/NN-*.js`.** Run `node --check` on the file.
4. **Open `dev/index.html` in a browser.** DevTools console must stay clean.
5. **Test the feature manually.** Click everything that touches it.
6. **If you change global state** (e.g. add a new top-level `let`), it goes in `dev/js/07-globals.js` — NOT scattered.
7. **Update `CHANGELOG.md`** with one line.
8. **Hand off to Dan for review and the dev → root mirror.**
...
*End of playbook. Last updated: 2026-05-02 after modular refactor.*
